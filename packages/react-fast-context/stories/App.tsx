...
const selectTodo = (state: AppState, id: number) =>
...
export const Item: FC<{ id: number }> = ({ id }) => {
  ...
  const selectCurrentTodo = useCallback(
    (state: AppState) => selectTodo(state, id),
    [id]
  )
  ...
  return (
    <li style={{ opacity: item.completed ? 0.4 : 1 }}>
      {item.name}
      <button onClick={() => markCompleted(item.id, !item.completed)}>
