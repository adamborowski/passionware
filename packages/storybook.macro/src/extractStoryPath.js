...
module.exports = {
  extractStoryPath: (
    ...
    const packageDirectory = path.normalize(
      path.resolve(actualPackagePath, '..')
    );

    const pathFromRootToPackage = path.normalize(
      path.relative(rootDirectory, packageDirectory)
    );
    const pathFromPackageToFile = path.normalize(
      path.relative(packageDirectory, storyPath)
    );

    const packagePath = pathFromRootToPackage.split(path.sep).slice(0, -1);
    const [storyKind, ...restOfPackagePath] = packagePath;

    const storyTitle = extractStoryTitleFromPathRelativeToPackage(
      pathFromPackageToFile,
      packageNameUnscoped
    );

    return `${_.capitalize(storyKind)}/${[
      restOfPackagePath.join(' '),
      packageNameUnscoped,
      ...storyTitle,
    ]
...
const stripIndexStories = (path, packageNameUnscoped) => {
  ...
  const stripForStoriesWithNameSameAsPackageName =
    pathCopy.length === 1 && pathCopy[0] === packageNameUnscoped;
  const stripForStoriesWithNameSameAsFolderName =
    pathCopy[length - 1] === pathCopy[length - 2];
  if (
    stripForIndexStories ||
    stripForStoriesWithNameSameAsPackageName |
      stripForStoriesWithNameSameAsFolderName
  ) {
    pathCopy.pop();
  }
  return pathCopy;
 ...
 */
const extractStoryTitleFromPathRelativeToPackage = (
  pathFromPackageToFile,
  packageNameUnscoped
) =>
  stripIndexStories(
